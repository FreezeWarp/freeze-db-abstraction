...
abstract class Database
{
        ...
        elseif (error_reporting() === 0) { // Errors are suppressed.
            ...
            if (!$suppressErrors) {
                if ($this->errorFormatFunction && function_exists($this->errorFormatFunction)) {
                    ...
                    throw new Exception('A database error has occurred: ' . $errorMessage . ' (other info: ' . print_r($errorData, true) . ') (query = ' . (is_string(end($this->queryLog)[0]) ? end($this->queryLog)[0] : 'not a string') . ').');
                }
            }
            else {
    ...
    /**
     * Gets the last serial ID used in an insert(). May or may not reset after an insert() that does not involve a serial ID.
     *
     * @return int
     */
     ...
     * @return bool     True on success, false on failure.
     */
    public abstract function upsert($tableName, $conditionArray, $dataArray, $dataArrayOnInsert = []);
